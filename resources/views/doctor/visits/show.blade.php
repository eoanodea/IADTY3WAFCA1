        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <form style="display:inline-block;float: right;margin: 0 5px;" method="POST" action="{{ route('doctor.visits.destroy', $visit->id) }}">
                                <input type="hidden" name="_method" value="DELETE">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <button type="submit" class="form-control btn btn-danger float-right">Delete</button>
                            </form>

                            <a href="{{ route('doctor.visits.edit', $visit->id) }}" class="btn btn-warning float-right">Edit</a>

                            @if(!$visit->cancelled && $visit->date > date('Y-m-d')) <a href="{{ route('doctor.visits.cancel', $visit->id) }}" class="btn btn-danger float-right" style="margin: 0 5px">Cancel Visit</a> @endif
                    </div>
            </div>
        </div>
    </div>
